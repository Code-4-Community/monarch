...
import { Badge, Box, Center, Divider, Flex, Grid, GridItem, Heading, HStack, Image, Input, InputGroup, InputLeftAddon, InputRightAddon, Link, SimpleGrid, Skeleton, Stack, StackItem, Stat, StatArrow, StatHelpText, StatLabel, StatNumber, Text, VStack, Wrap, WrapItem } from '@chakra-ui/react'
    ...
    <div>
      <div style={{ marginTop: 64 }}>
        ...
        <div style={{ marginBlock: 12 }}>
          <small>{(therapists != null) && `Found ${therapists?.length} matching practitioners.`}</small>
        </div>
      </div>
          ...
          <VStack spacing={5}>
            {
              ...
              (therapistsToRender.map(therapist =>
                <>
                  ...
                  <Box w='full' borderWidth='1px' borderRadius='lg' overflow='hidden' padding={5} gap={20}>
                      ...
                      <Wrap spacingX={6} spacingY={3} align='center'>
                        ...
                        <WrapItem>
                          <Box>
                            ...
                            <Text>
                              {(therapist.geocode != null) && (coords != null) ? (dist(therapist.geocode?.lat, therapist.geocode?.long, coords?.latitude, coords?.longitude)).toFixed(2) + ' miles away' : 'Unknown'}
                            </Text>
                          </Box>
                        </WrapItem>
                      </Wrap>
